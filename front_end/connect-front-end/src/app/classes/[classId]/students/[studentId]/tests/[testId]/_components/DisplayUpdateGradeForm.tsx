...
const DisplayUpdateGradeForm: FC<DisplayUpdateGradeFormProps> = ({
  ...
  if (error) {
    return (
      <div className={styles.updateGradeError}>
        {"Grade update failed, please try again"}
      </div>
    );
  }

  const UpdateGradeFormComponent = (
    <>
  ...
  return (
    <form
      className={styles.DisplayUpdateGradeFormContainer}
      onSubmit={handleGradeChange}
    >
      {newGrade === DEFAULT_GRADE ? (
        <div className={styles.DisplayUpdateGradeFormUpdatedGradeContainer}>
          {UpdateGradeFormComponent}
        </div>
      ) : (
        <div className={styles.DisplayUpdateGradeFormUpdatedGradeContainer}>
